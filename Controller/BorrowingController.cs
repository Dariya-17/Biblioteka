...
namespace Controller
{
    public class BorrowingController
    {
        ...
        public async Task<string> ReturnBookByTitle(string bookTitle, int readerId, DateTime chosenDate)
        {
            ...
            string cleanTitle = bookTitle.Trim().ToLower();
            bool bookExists = await context.Books.AnyAsync(b => b.Title.Trim().ToLower() == cleanTitle);
            if (!bookExists)
            {
            ...
            var borrowing = await context.Borrowings
                ...
                .FirstOrDefaultAsync(b => b.ReaderId == readerId &&
                                          b.Book.Title.Trim().ToLower() == cleanTitle &&
                                          b.ReturnedDate == null);
            if (borrowing == null)
            {
